    ...
    throws InterruptedException, IOException, FileNotFoundException {
        ...
        switch (commands[0]) {
            ...
            case "new" :
                if (Main.tm == null) {
                    output =
                        "Il faut définir une mémoire transactionnelle avant "+
                        "l'utiliser.";
            ...
            case "help" :
                output =
                    ...
                    "de s'avorter.\n"+
                    "  - 'run <script>' exécute ligne à ligne les commandes "+
                    "contenues dans un fichier script.\n"+
            ...
            default :
                if (Main.tm == null) {
                    output =
                        "N'oublie pas de créer ta mémoire transactionnelle "+
                        "avant de jouer avec.";
                } else if (!Main.tm.getTransactions().contains(commands[0])) {
                    output =
                        "Transaction inconnue au bataillon.";
                } else if (Main.blocked.contains(commands[0])) {
                    output =
                        "Essaye encore, cette transaction est bloquée.";
                } else {
                    Main.instructions
                    .get(commands[0])
                    .put(Arrays
